# Local Loom — Technical Requirements Document
...
LocalLoom/
  ...
  LocalLoom/
    App/
      LocalLoomApp.swift          — @main, MenuBarExtra + WindowGroup wiring
...
The standing bootstrap protocol (`~/.claude/skills/bootstrap-monorepo/SKILL.md`) assumes a pnpm/Turborepo web monorepo. Local Loom is a single native Xcode target with no workspace boundaries to enforce, so most of that tooling has no analogue here. What translates and what doesn't:
...
- **`RecordingStore`** — folder scan, save/delete/notify logic against a temp directory fixture. No real `~/Movies/LocalLoom` needed; inject the root path.
