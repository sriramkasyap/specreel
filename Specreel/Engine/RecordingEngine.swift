...
private let engineLog = Logger(subsystem: "dev.yagna.specreel", category: "RecordingEngine")
...
enum RecordingEngineError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .screenRecordingDenied:
            return "Screen Recording permission is required. Enable Specreel in System Settings, then quit and reopen the app."
...
final class RecordingEngine: @unchecked Sendable {
    ...
    fileprivate let pipelineQueue = DispatchQueue(label: "dev.yagna.specreel.pipeline", qos: .userInitiated)
    ...
    func start(config: RecordingConfig) async throws {
        ...
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("Specreel-\(UUID().uuidString).mp4")
        outputURL = url
    ...
    private func setupWebcam(deviceID: String?) throws {
        ...
        webcamProxy = proxy
        output.setSampleBufferDelegate(proxy, queue: DispatchQueue(label: "Specreel.Webcam", qos: .userInitiated))
