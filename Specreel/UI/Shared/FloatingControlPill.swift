...
struct FloatingControlPill: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            Circle()
                .fill(engine.phase == .paused ? Color.orange : SpecreelTheme.record)
            ...
            Button(action: onStop) {
                Image(systemName: "stop.fill")
                    ...
                    .frame(width: 22, height: 22)
                    .background(SpecreelTheme.record, in: RoundedRectangle(cornerRadius: 5, style: .continuous))
            }
            .buttonStyle(.plain)
            .help("Stop")
        }
        ...
        .padding(.vertical, 8)
        .background(SpecreelTheme.pillFill, in: Capsule())
