...
struct RecordingConfigView: View {
    ...
    private func expandedBody(config: RecordingConfig) -> some View {
        ...
        .background(SpecreelTheme.inspectorBackground)
    }
    ...
    private func requestScreenRecordingAccess() {
        ...
        if granted {
            ...
            isPermissionError = true
            loadError = "Screen Recording permission was not granted. Enable Specreel in System Settings, then quit and reopen the app."
            ScreenCaptureAccess.openSystemSettings()
        }
    }
    ...
    private func applyLoadError(_ error: Error) {
        ...
        if Self.isScreenRecordingPermissionError(raw) {
            isPermissionError = true
            loadError = "Screen Recording permission is required. Enable Specreel in System Settings → Privacy & Security → Screen Recording, then quit and reopen the app."
        } else {
            isPermissionError = false
            loadError = raw
        }
